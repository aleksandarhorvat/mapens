...
[A] Replay task (simulated clock, speed ×N) ─► pipeline.process(message)
...
5. `confidence` = fused score normalized to 0–1 (× 0.5 if pair distance > 800 m). If `confidence < THRESHOLD` (start 0.35, tune on gold set) → unresolved.
...
5. React + Leaflet frontend (first against mock JSON matching §6.6, then real API).
...
- Day 1: agree on §6 contracts and label guidelines, write `aliases.json`.
...
1. **Respect the contracts in §6.** If a change is needed, say so explicitly and tell the user to inform the other person and update this file.
...
4. Stay within the owner's files unless asked; if the other person's module is missing, use the mocks from §9.
...
7. After finishing a task, output a short **PROGRESS.md entry** (format in §12) the user can paste.
